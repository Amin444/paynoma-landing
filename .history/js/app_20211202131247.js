window.addEventListener('scroll', function(){
  ...
  const mobi = document.querySelector('.open-mobi-menu')
  const close = document.querySelector('.close-mobi-menu')
  header.classList.toggle('fixed', window.scrollY > 100)  
  mobi.classList.toggle('fixed2', window.scrollY > 100)  
  close.classList.toggle('fixed2', window.scrollY > 100)  
  handleWindowScroll();
  setInterval(() => {
    animOnScroll()
  }, 500);
...
const PaymonaBI =() => {
  ...
  const openMenu = () =>{
    document.querySelector('.open-mobi-menu').style.display = 'flex';
    document.querySelector('.bag-dark').style.display = 'block';
  }
